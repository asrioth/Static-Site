...
3. This is another ordered item"""
    ...
    def test_get_block_type_text_node(self):
        block, children = TextBlockParser.get_block_type_text_node(TextBlockType.CODE, "```a```")
        expected_block = "a"
        expected_children = [TextNode("```", TextType.TEXT), TextNode("```", TextType.TEXT)]
        self.assertEqual(block, expected_block)
        self.assertEqual(children, expected_children)
        block, children = TextBlockParser.get_block_type_text_node(TextBlockType.HEADING, "# a")
        expected_children = [TextNode("# ", TextType.TEXT)]
        self.assertEqual(block, expected_block)
        self.assertEqual(children, expected_children)
        block, children = TextBlockParser.get_block_type_text_node(TextBlockType.HEADING, "###### a")
        expected_children = [TextNode("###### ", TextType.TEXT)]
        self.assertEqual(block, expected_block)
        self.assertEqual(children, expected_children)
        block, children = TextBlockParser.get_block_type_text_node(TextBlockType.ORDERED_LIST, "1. a")
        expected_children = [TextNode("1. ", TextType.TEXT)]
        self.assertEqual(block, expected_block)
        self.assertEqual(children, expected_children)
        block, children = TextBlockParser.get_block_type_text_node(TextBlockType.PARAGRAPH, "a")
        expected_children = []
        self.assertEqual(block, expected_block)
        self.assertEqual(children, expected_children)
        block, children = TextBlockParser.get_block_type_text_node(TextBlockType.QUOTE, ">a")
        expected_children = [TextNode(">", TextType.TEXT)]
        self.assertEqual(block, expected_block)
        self.assertEqual(children, expected_children)
        block, children = TextBlockParser.get_block_type_text_node(TextBlockType.UNORDERED_LIST, "* a")
        expected_children = [TextNode("* ", TextType.TEXT)]
        self.assertEqual(block, expected_block)
        self.assertEqual(children, expected_children)
    ...
    def test_block_to_children_nodes(self):
        ...
        expected_children = [LeafNode(None, "```"), LeafNode(None, self.block4[3:-3]), LeafNode(None, "```")]
        self.assertEqual(children, expected_children)
        children = TextBlockParser.block_to_children_nodes(self.block1, TextBlockType.HEADING)
        expected_children = [LeafNode(None, "# "), LeafNode(None, self.block1[2:])]
        self.assertEqual(children, expected_children)
        children = TextBlockParser.block_to_children_nodes("#####" + self.block1, TextBlockType.HEADING)
        expected_children = [LeafNode(None, "###### "), LeafNode(None, self.block1[2:])]
        self.assertEqual(children, expected_children)
        children = TextBlockParser.block_to_children_nodes(self.block6.split("\n")[0], TextBlockType.ORDERED_LIST)
        expected_children = [LeafNode(None, "1. "), LeafNode(None, self.block6.split("\n")[0][3:])]
        self.assertEqual(children, expected_children)
        ...
        expected_children = [LeafNode(None, ">"), LeafNode(None, self.block5.split("\n")[0][1:])]
        self.assertEqual(children, expected_children)
        children = TextBlockParser.block_to_children_nodes(self.block3.split("\n")[0], TextBlockType.UNORDERED_LIST)
        expected_children = [LeafNode(None, "* "), LeafNode(None, self.block3.split("\n")[0][2:])]
        self.assertEqual(children, expected_children)

    def test_block_to_html_node(self):
        parent_node = TextBlockParser.block_to_html_node(self.block1)
        expected_parent_node = ParentNode("h1", [LeafNode(None, "# "), LeafNode(None, self.block1[2:])])
        ...
        expected_parent_node = ParentNode("ul", [ParentNode("li", [LeafNode(None, "* "), LeafNode(None, line1[2:])]), ParentNode("li", [LeafNode(None, "* "), LeafNode(None, line2[2:])]), ParentNode("li", [LeafNode(None, "* "), LeafNode(None, line3[2:])])])
        self.assertEqual(repr(parent_node), repr(expected_parent_node))
        parent_node = TextBlockParser.block_to_html_node(self.block4)
        expected_parent_node = ParentNode("pre", [ParentNode("code", [LeafNode(None, "```"), LeafNode(None, self.block4[3:-3]), LeafNode(None, "```")])])
        self.assertEqual(repr(parent_node), repr(expected_parent_node))
        parent_node = TextBlockParser.block_to_html_node(self.block5)
        line1, line2, line3 = self.block5.split("\n")
        expected_parent_node = ParentNode("blockquote", [LeafNode(None, ">"), LeafNode(None, line1[1:]), LeafNode(None, ">"), LeafNode(None, line2[1:]), LeafNode(None, ">"), LeafNode(None, line3[1:])])
        self.assertEqual(repr(parent_node), repr(expected_parent_node))
        parent_node = TextBlockParser.block_to_html_node(self.block6)
        line1, line2, line3 = self.block6.split("\n")
        expected_parent_node = ParentNode("ol", [ParentNode("li", [LeafNode(None, "1. "), LeafNode(None, line1[3:])]), ParentNode("li", [LeafNode(None, "2. "), LeafNode(None, line2[3:])]), ParentNode("li", [LeafNode(None, "3. "), LeafNode(None, line3[3:])])])
        self.assertEqual(repr(parent_node), repr(expected_parent_node))
    ...
    def build_full_html_structure(self):
        ...
        block1_node = ParentNode("h1", [LeafNode(None, "# "), LeafNode(None, self.block1[2:])])
        block2_part1, block2_part2, block2_part3, block2_part4, block2_part5 = "This is a paragraph of text. It has some ", "bold", " and ", "italic", " words inside of it."
        ...
        line1, line2, line3 = self.block3.split("\n")
        block3_node = ParentNode("ul", [ParentNode("li", [LeafNode(None, "* "), LeafNode(None, line1[2:])]), ParentNode("li", [LeafNode(None, "* "), LeafNode(None, line2[2:])]), ParentNode("li", [LeafNode(None, "* "), LeafNode(None, line3[2:])])])
        block4_node = ParentNode("pre", [ParentNode("code", [LeafNode(None, "```"), LeafNode(None, self.block4[3:-3]), LeafNode(None, "```")])])
        line1, line2, line3 = self.block5.split("\n")
        block5_node = ParentNode("blockquote", [LeafNode(None, ">"), LeafNode(None, line1[1:]), LeafNode(None, ">"), LeafNode(None, line2[1:]), LeafNode(None, ">"), LeafNode(None, line3[1:])])
        line1, line2, line3 = self.block6.split("\n")
        block6_node = ParentNode("ol", [ParentNode("li", [LeafNode(None, "1. "), LeafNode(None, line1[3:])]), ParentNode("li", [LeafNode(None, "2. "), LeafNode(None, line2[3:])]), ParentNode("li", [LeafNode(None, "3. "), LeafNode(None, line3[3:])])])
